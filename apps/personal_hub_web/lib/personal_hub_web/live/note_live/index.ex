  ...
  def render(assigns) do
    ...
    <div class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="flex justify-between items-center">
        ...
        <.link navigate={~p"/notes/new"} class="px-4 py-2 rounded-xl text-sm font-medium text-white bg-primary hover:bg-primary/90 transition-colors">
          New Note
        </.link>
      </div>
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          <.form for={@form} id="note-form" phx-submit="save" class="space-y-5">
            ...
            <div class="flex items-center gap-3 pt-2">
              ...
              <.link navigate={~p"/notes"} class="text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
                Cancel
              </.link>
            </div>
          </.form>
        ...
        <%= for note <- @notes do %>
          <div class={[
            ...
            <div class="flex items-start gap-2 mb-2">
              <%= if note["pinned"] do %>
                <span class="rounded-full px-2 py-0.5 text-xs font-medium bg-amber-100 text-amber-700">Pinned</span>
              <% end %>
            </div>
            <h2 class="text-base font-semibold text-gray-900">{note["title"]}</h2>
            <p class="text-sm text-gray-500 mt-1.5 line-clamp-3">{String.slice(note["content"] || "", 0..150)}</p>
            ...
            <div class="flex items-center gap-3 mt-3 pt-3 border-t border-gray-100">
              <button phx-click="toggle_pin" phx-value-id={note["id"]} class="text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors">
                <%= if note["pinned"], do: "Unpin", else: "Pin" %>
              </button>
              <.link navigate={~p"/notes/#{note["id"]}/edit"} class="text-sm font-medium text-gray-600 hover:text-gray-900 transition-colors">Edit</.link>
              <button phx-click="delete" phx-value-id={note["id"]} class="text-sm font-medium text-red-500 hover:text-red-700 transition-colors"
                data-confirm="Delete this note?">Delete</button>
            </div>
          </div>
        <% end %>
