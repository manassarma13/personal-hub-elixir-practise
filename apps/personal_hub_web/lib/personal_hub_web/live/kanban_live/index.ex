defmodule PersonalHubWeb.KanbanLive.Index do
  ...
  def render(assigns) do
    ...
    <div id="local-store" phx-hook="LocalStore" phx-update="ignore" data-collections="posts,notes,tasks"></div>
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
          <button
            ...
            class={[
              ...
              if(@view == "kanban", do: "bg-white shadow-sm text-gray-900", else: "text-gray-500 hover:text-gray-700")
            ]}
          >
            <.icon name="hero-view-columns" class="size-4 inline mr-1" />
            Kanban
          </button>
          <button
            phx-click="toggle_view"
            ...
            class={[
              "px-3 py-1.5 rounded-lg text-sm font-medium transition-colors",
              if(@view == "calendar", do: "bg-white shadow-sm text-gray-900", else: "text-gray-500 hover:text-gray-700")
            ]}
          >
            <.icon name="hero-calendar-days" class="size-4 inline mr-1" />
            Calendar
          </button>
        </div>
      </div>
          ...
          <%= for status <- ["todo", "in_progress", "done"] do %>
            <div class={["rounded-2xl border p-4 min-h-[400px]", status_column_class(status)]}>
              <div class="flex items-center justify-between mb-4">
                <span class={["px-3 py-1 rounded-full text-sm font-semibold", status_header_class(status)]}>
                  {status_label(status)}
                </span>
                ...
                <%= for task <- tasks_by_status(@tasks, status) do %>
                  <div class="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-start gap-2">
                      <span class={["mt-1.5 w-2 h-2 rounded-full shrink-0", priority_dot(task["priority"])]}></span>
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-2">
                          ...
                          <%= if task["due_date"] && task["due_date"] != "" do %>
                            <span class={[
                              "text-xs font-medium",
                              if(status != "done" && due_soon?(task["due_date"]), do: "text-red-500", else: "text-gray-400")
                            ]}>
                    ...
                    <div class="flex items-center gap-2 mt-3 pt-2 border-t border-gray-100">
                      <%= if status == "todo" do %>
                        <.link navigate={~p"/tasks"} class="text-xs font-medium text-blue-600 hover:text-blue-800">Start</.link>
                      <% end %>
                      <%= if status != "done" do %>
                        <.link navigate={~p"/tasks"} class="text-xs font-medium text-green-600 hover:text-green-800">Complete</.link>
                      <% end %>
                      <.link navigate={~p"/tasks/#{task["id"]}/edit"} class="text-xs font-medium text-gray-500 hover:text-gray-700">Edit</.link>
                    </div>
                  </div>
                <% end %>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-4">
              ...
              <.link navigate={~p"/posts"} class="text-xs font-medium text-primary hover:underline">View all</.link>
            </div>
            <div class="space-y-2">
              <%= for post <- Enum.take(@posts, 5) do %>
                <.link navigate={~p"/posts/#{post["id"]}"} class="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-gray-50 transition-colors">
                  <span class="text-sm text-gray-700 truncate flex-1">{post["title"]}</span>
                  <span class="text-xs text-gray-400 shrink-0 ml-2">{relative_time(post["inserted_at"])}</span>
                </.link>
              <% end %>
              <%= if @posts == [] do %>
          ...
          <div class="bg-white rounded-2xl border border-gray-200 p-5">
            <div class="flex items-center justify-between mb-4">
              ...
              <.link navigate={~p"/notes"} class="text-xs font-medium text-primary hover:underline">View all</.link>
            </div>
            <div class="space-y-2">
              <%= for note <- Enum.take(@notes, 5) do %>
                <div class="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <span class="text-xs text-gray-400 shrink-0 ml-2">{relative_time(note["inserted_at"])}</span>
                </div>
              <% end %>
              <%= if @notes == [] do %>
      ...
      <%= if @view == "calendar" do %>
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <div class="flex items-center justify-between p-4 border-b border-gray-100">
            <button phx-click="prev_month" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
            ...
            <div class="flex items-center gap-3">
              ...
              <button phx-click="today" class="px-2.5 py-1 rounded-lg text-xs font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                Today
              </button>
            </div>
            <button phx-click="next_month" class="p-1.5 rounded-lg hover:bg-gray-100 transition-colors">
                  ...
                  <div class={[
                    ...
                    <div class="flex items-center justify-between mb-1">
                      <span class={[
                        ...
                        if(day == @today, do: "bg-primary text-white w-6 h-6 rounded-full flex items-center justify-center", else: if(day.month != @current_month.month, do: "text-gray-300", else: "text-gray-700"))
                      ]}>
                        {day.day}
                      </span>
                    </div>
                    ...
                    <div class="space-y-0.5">
                      <%= for item <- Enum.take(items, 3) do %>
                        <div class={[
                          ...
                          cond do
                            item.type == :task && item[:status] == "done" -> "bg-green-100 text-green-700"
                            item.type == :task && item[:status] == "in_progress" -> "bg-blue-100 text-blue-700"
                            item.type == :task -> "bg-amber-100 text-amber-700"
                            item.type == :post -> "bg-violet-100 text-violet-700"
                            item.type == :note -> "bg-sky-100 text-sky-700"
        ...
        <div class="flex flex-wrap items-center gap-4 px-1">
          ...
          <span class="flex items-center gap-1.5 text-xs text-gray-500">
            <span class="w-3 h-3 rounded bg-violet-100 border border-violet-200"></span>
            Post
          </span>
          <span class="flex items-center gap-1.5 text-xs text-gray-500">
