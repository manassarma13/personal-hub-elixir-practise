    ...
    <div class="max-w-3xl mx-auto space-y-6">
      <%= if @post do %>
        <div class="flex justify-between items-center">
          <.link navigate={~p"/posts"} class="inline-flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors">
            <.icon name="hero-arrow-left" class="size-4" />
            Back to Posts
          </.link>
          <.link navigate={~p"/posts/#{@post["id"]}/edit"} class="px-4 py-2 rounded-xl text-sm font-medium text-gray-700 border border-gray-200 hover:bg-gray-50 transition-colors">
            Edit
          </.link>
        </div>
