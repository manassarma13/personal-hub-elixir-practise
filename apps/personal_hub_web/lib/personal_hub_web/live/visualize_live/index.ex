defmodule PersonalHubWeb.VisualizeLive.Index do
    ...
    "sales" => %{
      ...
      data: %{
        ...
        datasets: [
          %{
            label: "Revenue ($K)",
            data: [120, 190, 170, 240, 210, 280],
            backgroundColor: "rgba(255, 56, 92, 0.6)"
          },
          %{
            label: "Expenses ($K)",
            data: [80, 110, 130, 150, 120, 160],
    ...
    "temperature" => %{
      ...
      data: %{
        ...
        datasets: [
          %{
            label: "New York (°C)",
            data: [-1, 0, 5, 12, 18, 23, 26, 25, 21, 14, 8, 2],
          ...
          %{
            label: "London (°C)",
            data: [5, 5, 7, 10, 13, 16, 18, 18, 15, 11, 8, 5],
          ...
          %{
            label: "Tokyo (°C)",
            data: [6, 6, 10, 15, 20, 23, 27, 28, 24, 18, 13, 8],
  ...
  defp normalize_chart_data(parsed) do
    ...
    cond do
      ...
      Map.has_key?(atom_keys, :rows) and Map.has_key?(atom_keys, :cols) and
          Map.has_key?(atom_keys, :values) ->
        atom_keys
          ...
          <div class="lg:col-span-1 space-y-4">
            ...
            <div class="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <form phx-submit="parse_json" class="space-y-3">
                ...
                <button
                  type="submit"
                  class="w-full px-3 py-2 rounded-xl text-xs font-medium text-white bg-primary hover:bg-primary/90 transition-colors"
                >
                  Render JSON
                </button>
              </form>
              ...
              <%= if @chart_data == nil do %>
                <div class="text-center space-y-3">
                  ...
                  <p class="text-sm text-gray-500">
                    Select a demo dataset or upload JSON to get started
                  </p>
                </div>
              <% else %>
                      ...
                      <table class="w-full">
                        ...
                        <tbody>
                          <%= for {row_label, row_idx} <- Enum.with_index(Map.get(@chart_data, :rows, [])) do %>
                            <tr>
                              <td class="p-2 text-xs text-gray-500 font-medium text-right">
                                {row_label}
                              </td>
                              <%= for {val, _col_idx} <- Enum.with_index(Enum.at(Map.get(@chart_data, :values, []), row_idx, [])) do %>
                                <td class={[
                                  ...
                                  heatmap_color(val, max_heatmap_value(@chart_data))
                                ]}>
                                  {val}
                                </td>
                              <% end %>
                ...
                <% else %>
                  <div class="w-full">
                    <canvas id="chart-canvas" phx-hook="ChartJS" phx-update="ignore" class="w-full">
                    </canvas>
                  </div>
                <% end %>
              <% end %>
